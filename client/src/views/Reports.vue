...
export default {
  ...
  setup() {
    ...
    // Best quarter = highest fulfillment rate; revenue breaks ties
    const bestQuarter = computed(() => {
      if (!quarterlyData.value.length) return ''
      return quarterlyData.value.reduce((best, q) => {
        if (q.fulfillment_rate > best.fulfillment_rate) return q
        if (q.fulfillment_rate === best.fulfillment_rate && q.total_revenue > best.total_revenue) return q
        return best
      }).quarter
    })
